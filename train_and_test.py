...
import math
from models import CBPE_Net

class UCI_Dataset(Dataset):
    def __init__(self, root_path, window_L=88):
        self.root_path = root_path
        self.w_L = window_L
        self.mat_file_list = [] 
        self.file_name_list = [f"part_{i}.mat" for i in range(1, 2)]
        ...
        for i in range(len(self.file_name_list)):
            ...
            for j in range(1000):
                current_record_number = mat_file[j].shape[1] - self.w_L - 1
    ...
    def __getitem__(self, idx):
        ...
        ppg = self.mat_file_list[mat_file_index][record_index][0, idx: idx+self.w_L] 
        ppg = (ppg - np.mean(ppg)) / np.std(ppg)
        ecg = self.mat_file_list[mat_file_index][record_index][2, idx: idx+self.w_L] 
        ecg = (ecg - np.mean(ecg)) / np.std(ecg)
        abp = self.mat_file_list[mat_file_index][record_index][1, idx+self.w_L-1] 
        return torch.tensor(np.stack([ecg, ppg], axis=0), dtype=torch.float32), torch.tensor(abp, dtype=torch.float32)
    
dataset_file_path = './UCI_Datasets'
model = CBPE_Net(in_channels=2, deep_supervision=True)
